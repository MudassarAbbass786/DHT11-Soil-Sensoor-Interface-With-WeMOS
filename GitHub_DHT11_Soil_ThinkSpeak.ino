...
String apiWritekey = "xxxxxxxxxxxxxxx";   // Api key is used to write your sensor vlaues onto the thingspeak channel
const char* ssid = "*******";           // the name of the WiFi network you want to connect to
const char* password = "*********" ;       // password of router
...
void loop() {
  ...
  if (client.connect(server,80))         // Connects to a specified IP address and port. The return value indicates success or failure. "184.106.153.149" or api.thingspeak.com
  {
    ...
    tsData +="&field2=";                 // upload data into field 2 
    tsData += String(h);                 // upload humidity values into field 2
    tsData +="&field3=";                 // upload data into field 3 
    tsData += String(percentValue);      // upload Moisture level values into field 3
    ...
    Serial.print(t);
    Serial.print("\"Moisture\": ");    // print the Moisture level values to serial monitor
